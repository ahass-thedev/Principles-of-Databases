...
<%
	try {
		...
		String str = "SELECT * FROM Reservation WHERE Res_No = '" + Res_No + "'"+ " AND" +" "+ "Account_No = '"+ Account_No+ "'";

		ResultSet result = stmt.executeQuery(str);

		if (!result.next() ) {
		con.close();
		throw new IndexOutOfBoundsException("Account or Res_No does not exist");
	}
		


 			//Make a SELECT query for Reservation
			str = "SELECT * FROM Reservation WHERE Account_No='"+Account_No+"'"+" "+"AND"+" "+ "Res_No = "+"'"+Res_No+"'";

			//Run the query against the database.
			 result = stmt.executeQuery(str);

			out.print("<h1>Reserved flights for Account Number: "+ Account_No +"</h1>");
			...
			**/
			out.print("\n\n\n\nCancel Reservation for only Business or First Class Tickets:\n");
			...
			while(result.next()){
				...
				out.print("<button value="+result.getString("Res_No")+">"+result.getString("Res_No")+"</button>");
				out.print("<input type=\"submit\" value=\"delete\"class=\"login100-form-btn wrap-input100 m-b-16\"style=\"width:350px\">");
				out.print("</form>");
					}}
